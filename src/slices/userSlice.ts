...
export const loginUserAsync = ({ credentials, history }: IUserParams): AppThunk => async (dispatch: AppDispatch) => {
    ...
    try {
        ...
        const { data: userData }: { data: IUserResponce } = await apiService.get(BACKEND_ROUTING.AUTH.USER, {
            headers: {
        ...
        storeToast({
            status: 'success',
            title: 'Logowanie',
            message: `${capitalize(userData.username)}, witamy w naszym serwisie ponownie`,
        });
    } catch (error) {
        storeToast({
            status: 'error',
